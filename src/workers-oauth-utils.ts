...
// Vendored verbatim from Cloudflare's reference MCP-with-Access demo
// (cloudflare/ai/demos/remote-mcp-cf-access). Keep changes minimal so it can
...
/**
 * Validates a URL for security. Uses whitelist security: only allows https: and http:
 * schemes. All other schemes (javascript:, data:, file:, etc.) are rejected.
 *
...
export interface ApprovalDialogOptions {
	...
	/** Arbitrary state data passed through the approval flow and returned on completion. */
 ...
 * The dialog displays information about the client and server and includes a form
 * to submit approval with CSRF protection.
 */
      ...
      <head>
        ...
        <style>
          ...
          body {
            ...
            line-height: 1.6; color: var(--text-color); background-color: #f9fafb; margin: 0; padding: 0;
          }
        ...
        <div class="container">
          ...
          <div class="card">
            ...
            <div class="client-info">
              ...
              ${
					clientUri
						? `<div class="client-detail"><div class="detail-label">Website:</div><div class="detail-value small"><a href="${clientUri}" target="_blank" rel="noopener noreferrer">${clientUri}</a></div></div>`
						: ""
				}
              ${
					policyUri
						? `<div class="client-detail"><div class="detail-label">Privacy Policy:</div><div class="detail-value"><a href="${policyUri}" target="_blank" rel="noopener noreferrer">${policyUri}</a></div></div>`
						: ""
				}
              ${
					tosUri
						? `<div class="client-detail"><div class="detail-label">Terms of Service:</div><div class="detail-value"><a href="${tosUri}" target="_blank" rel="noopener noreferrer">${tosUri}</a></div></div>`
						: ""
				}
              ${
					redirectUris.length > 0
						? `<div class="client-detail"><div class="detail-label">Redirect URIs:</div><div class="detail-value small">${redirectUris
								.map((uri) => `<div>${uri}</div>`)
								.join("")}</div></div>`
						: ""
				}
              ${
					contacts
						? `<div class="client-detail"><div class="detail-label">Contact:</div><div class="detail-value">${contacts}</div></div>`
						: ""
				}
            </div>
 ...
 * Returns [access_token, id_token, null] on success or [null, null, Response] on error.
 */
